...
public class teampj01 {
	public static void main(String[] args) {
		...
		int number = 1;// 다음 번호 boardArray[i][0]
		int count = 0;
		...
		while (run) {
			...
			if (selectMenu.equals("1")) {
				...
				for (int i = 0; i < boardArray.length; i++) {
					if (boardArray[i][0] == null) {
						boardArray[i][0] = String.valueOf(number); //String 형 변환
				...
				for (int i = 0; i < 100; i++) {
					if (boardArray[i][0] != null) {
						if (boardArray[i][0].equals(selectNumber)) {
							...
							int view = Integer.parseInt(boardArray[i][4]) + 1; //String 형이라 Int로 형 변환 후 +1
			...
			//System.arraycopy(복사할배열, 시작인덱스, 붙여넣을배열, 시작인덱스, 복사할인덱스개수)
